...
jq '.discord_bot_token = "'"$DISCORD_BOT_TOKEN"'"' $FILE > "${FILE}.tmp"
cat "${FILE}.tmp" > $FILE
jq '.discord_server_id = "'"$DISCORD_SERVER_ID"'"' $FILE > "${FILE}.tmp"
cat "${FILE}.tmp" > $FILE
jq '.database_host = "'"$DB_HOST"'"' $FILE > "${FILE}.tmp"
cat "${FILE}.tmp" > $FILE
...
cat "${FILE}.tmp" > $FILE
jq '.database_password = "'"$DB_PASSWORD"'"' $FILE > "${FILE}.tmp"
cat "${FILE}.tmp" > $FILE
jq '.database_name = "'"$DB_NAME"'"' $FILE > "${FILE}.tmp"
cat "${FILE}.tmp" > $FILE
